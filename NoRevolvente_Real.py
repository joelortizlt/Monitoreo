...
class NoRevolventeReal():
    ...
    def __init__(self,csv,mincosecha='',maxcosecha=''): #se insume un documento de Excel
        ...
        df_real['prepagos'] = pd.DataFrame({'pd':df_real.iloc[:,fn.encontrar_encabezado(df_real,'PREPAGO_1'):fn.encontrar_encabezado(df_real,'MTODESEMBOLSADO_1')].values.tolist()})
        df_real['desembolso'] = pd.DataFrame({'pd':df_real.iloc[:,fn.encontrar_encabezado(df_real,'MTODESEMBOLSADO_1'):fn.encontrar_encabezado(df_real,'prepagos')].values.tolist()})
        
        #seleccionar solo la data relevante
        df_real = df_real[fn.all_cortes(df_real)+['CODSOLICITUD','COSECHA','FAIL_TYPE', 'SURVIVAL','maxmad','prepagos','desembolso']]
        if mincosecha!='':
    ...
    def condensar(self,cortes=[]): #se insume una lista con los cortes que se desea
        ...
        for i in range(len(curvas)):
            ...
            curvas.at[i,'pd_real'] = fn.porcentaje(resultado)

            resultado = vector['can_final'].cumsum()
            curvas.at[i,'can_real'] = fn.porcentaje(resultado)
            
            #prepagos reales
            temp['sum_prepagos']=list(map(fn.operation_pd, temp['maxmad'], temp['prepagos']))
            temp['sum_desembolso']=list(map(fn.operation_pd, temp['maxmad'], temp['desembolso']))    
            a = fn.aggr_sum(temp['sum_prepagos'])
            b = fn.aggr_sum(temp['sum_desembolso'])
            resultado = [ai / bi for ai, bi in zip(a, b)]
            resultado = np.cumsum(resultado)
            curvas.at[i,'pre_real'] = fn.porcentaje(resultado)
