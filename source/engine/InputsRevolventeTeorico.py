...
class InputsRevolventeTeorico():
    ...
    def __init__(self,df,mincosecha='',maxcosecha=''): #se insume un documento de Excel
        ...
        df_teorico['saldo_marginal'] = pd.DataFrame({'pd':df_teorico.iloc[:,f.encontrar_encabezado(df_teorico,'SAL1'):f.encontrar_encabezado(df_teorico,'CAN1')].values.tolist()})
        df_teorico['can_marginal'] = pd.DataFrame({'pd':df_teorico.iloc[:,f.encontrar_encabezado(df_teorico,'CAN1'):f.encontrar_encabezado(df_teorico,'PD1')].values.tolist()})
        df_teorico['pd_marginal'] = pd.DataFrame({'pd':df_teorico.iloc[:,f.encontrar_encabezado(df_teorico,'PD1'):f.encontrar_encabezado(df_teorico,'saldo_marginal')].values.tolist()})
 
        #Se selecciona solo los campos relevantes y se filtra por cosecha
        df_teorico = df_teorico[f.all_cortes(df_teorico)+['CODCLAVEOPECTA','COSECHA','MAXMAD','pd_marginal', 'can_marginal', 'saldo_marginal']]
        if mincosecha!='':
    ...
    def condensar(self,cortes=[]): #se insume una lista con los cortes que se desea
        ...
        for i in range(len(curvas)):
            ...
            temp = pd.merge(self.df_teorico[cortes+['CODCLAVEOPECTA','COSECHA','MAXMAD','pd_marginal','can_marginal','saldo_marginal']], pd.DataFrame([curvas.loc[i,:]]), how='inner', left_on=cortes, right_on=cortes)
